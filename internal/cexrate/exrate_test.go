...
func Test_findMma(t *testing.T) {
	...
	bmin, smin, bmax, smax, bavg, savg, err := findMma(b)
	...
	bmin, smin, bmax, smax, bavg, savg, err = findMma(b)
	assert.Equal(t, err, errors.New("empty branches"))
	assert.Equal(t, bmin, 0.0)
	assert.Equal(t, smin, 0.0)
	assert.Equal(t, bmax, 0.0)
	assert.Equal(t, smax, 0.0)
	assert.Equal(t, bavg, 0.0)
	assert.Equal(t, savg, 0.0)
}
